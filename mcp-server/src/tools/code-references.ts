...
export function registerCodeReferenceTools(server: McpServer, apiClient: ApiClient): void {
  server.tool(
    "add_code_reference",
  ...
  server.tool(
    ...
    async (params) => {
      try {
        await apiClient.del(`/tickets/${params.ticket_id}/code-references/${params.reference_id}`);
        return {
